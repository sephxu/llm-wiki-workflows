...
// args 格式: { files: [{ path: "raw/sources/...", direction: "知识工程" }, ...] }
// path 支持相对路径（相对 WIKI_ROOT）或绝对路径
const FILES = (args?.files || []).map(f => ({
  path: f.path.startsWith('/') ? f.path : `${WIKI_ROOT}/${f.path}`,
  direction: f.direction || '跨方向',
}))
...
const today = args?.date || '2026-06-10'
...
4. 列出所有相关的现有页面 slug
5. 源文件是外部文档摘要用 source 类型，月报/周报/跨方向总结用 synthesis 类型
6. 页面标题和内容使用中文`, {
